...
def processUniques():
    if os.path.isfile(name_of_file):  # check for path
        ...
        for word in text.split():  # loop through each word from file
            ...
            uniqueWords.update({word: frequency})
            # frequency = 1  # initialize frequency of word to 1
            # if word not in uniqueWords:  # if word not found in dictionary
            #     uniqueWords.update({word: frequency})  # add word and frequency to dictionary
            # else:
